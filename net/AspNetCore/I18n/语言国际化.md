...
(2) 在HttpContext中再包裹了`RequestCulture`提供如Default之类的东西
...
官网中对扩展程序的说明：https://learn.microsoft.com/zh-cn/aspnet/core/fundamentals/localization-extensibility?view=aspnetcore-8.0
...
在默认的国际化实现方案中，是通过资源文件存储键值对，这并不是一个比较合适操作的方式，虽然文档中列了po文件替代资源文件的方式，但是最好的方式是（1）首先通过json文件存储资源（2）可以通过db的方式对前者进行一定的补充。
